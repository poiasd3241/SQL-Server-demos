...
namespace SqlServerDemo1
{
    ...
    Name	NVARCHAR (100) NOT NULL	Tokyo

    - Country table:
    ID				INT IDENTITY (1, 1) NOT NULL PK
    Name			NVARCHAR (100) NOT NULL
    Alpha3Code		CHAR (3) NOT NULL
    CapitalCityID	INT NOT NULL FK_Country_CapitalCityID_City
    Population		INT NULL
    Area			DECIMAL (10, 2) NULL
    UpdatedOn		DATETIME2 (3) DEFAULT (getutcdate()) NOT NULL

    Not supporting:
    - Tables with the same name in more than one schema (example: dbo.City, dbx.City)
    - City & Country in different schemas (example: dbo.City & dbx.Country)
    - Extra columns that don't have "NULL" or "DEFAULT". 
      Tables City and Country may contain columns that are not required for the app's functionality, 
      but they must be ignorable during the app's interaction with the database.
    */
